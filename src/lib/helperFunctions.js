...
export const getCampaignPostObject = (campaign) => {
  let postObject = {
    ...
    productIds: campaign?.products?.map((product) => {
      return product?.productId;
    }),
    associationIds: campaign?.associations?.map((association) => {
...
export const getBeneficiariesPost = (beneficiaries, flags) => {
  ...
  if (flags?.isSelfSpousePremiumSelected) {
    beneficieryPostObject["spouse"] = beneficiaries["spouse"];
  }

  if (flags?.isTopUpSelfSpousePremiumSelected) {
    beneficieryPostObject["spouse"] = beneficiaries["spouse"];
  }
  ...
  if (flags?.isChild1PremiumSelected) {
    beneficieryPostObject["child1"] = beneficiaries["child1"];
  }
  if (flags?.isChild2PremiumSelected) {
